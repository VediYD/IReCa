...
def func_nn_ppo(observation_dimensions, num_actions):
    ...
    x = tf.keras.layers.Conv2D(25, (5, 5), strides=1, activation="tanh", padding='same',
                               kernel_regularizer=l1_l2(l1=l1_reg, l2=l2_reg))(x)
    x = tf.keras.layers.Conv2D(25, (3, 3), strides=1, activation="tanh", padding='same',
                               kernel_regularizer=l1_l2(l1=l1_reg, l2=l2_reg))(x)
    x = tf.keras.layers.Conv2D(25, (3, 3), strides=1, activation="tanh", padding='same',
                               kernel_regularizer=l1_l2(l1=l1_reg, l2=l2_reg))(x)
    ...
    x = tf.keras.layers.Dense(64, activation="tanh", kernel_regularizer=l1_l2(l1=l1_reg, l2=l2_reg))(x)
    action_logits = tf.keras.layers.Dense(num_actions, kernel_regularizer=l1_l2(l1=l1_reg, l2=l2_reg))(
        x)  # 通常策略网络的输出不需要激活函数
    ...
    x = tf.keras.layers.Dense(64, activation="tanh", kernel_regularizer=l1_l2(l1=l1_reg, l2=l2_reg))(x)
    value = tf.keras.layers.Dense(1, kernel_regularizer=l1_l2(l1=l1_reg, l2=l2_reg))(
        x)  # Dense 后直接接 squeeze，无需再在 Dense 层中使用 relu
