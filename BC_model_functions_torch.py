...
from overcooked_ai_py.mdp.actions import \
    Action  # !!!!! 是在这里找到的 Action 这个 Class, 但是这个脚本虽然描述了各种 action, 但并不包含 action_space
...
BC_SAVE_DIR = os.path.join(".",
                           "bc_runs_functions")  # 这行代码定义了一个变量 BC_SAVE_DIR，它表示 BC 模型保存的目录路径。通过将 DATA_DIR 路径与子目录 "bc_runs" 组合在一起，确定了保存模型的完整路径。 定义行为克隆模型保存的目录路径
...
DEFAULT_BC_PARAMS = {
    ...
    # DEFAULT_ENV_PARAMS 是一个字典，其中包含了环境的一个参数 horizon，其值为 400。 horizon 参数通常指的是环境的最大时间步数（episode length）。也就是说，每个episode可以进行的最大步数是 400 步。
    "mdp_fn_params": {},
...
if __name__ == "__main__":  # 这行代码检查当前脚本是否作为主程序运行。__name__ 是一个内置变量，如果当前脚本被直接执行，那么 __name__ 的值将是 "__main__"。这种检查方式常用于确保某些代码块仅在脚本被直接运行时执行，而不是在被导入为模块时执行
    ...
    evaluate_bc_model(model,
                      params)  # 这行代码调用函数 evaluate_bc_model() 评估训练好的行为克隆模型的性能。 model 是已经训练好的模型。 params 是用于评估的参数，与训练参数一致。
