...
class OvercookedEnv(object):
    ...
    The environment keeps track of the current state of the agent, updates it as the agent takes actions, and provides rewards to the agent.
    ...
    def __init__(
        ...
        start_state_fn (callable):      Function that returns start state for the MDP, called at each environment reset (start_state_fn (可调用)：返回 MDP 起始状态的函数，每次重置环境时调用)
    ...
    def dummy_step(self, joint_action, joint_agent_action_info=None, display_phi=False):
        ...
        origin_state = deepcopy(self.state)
